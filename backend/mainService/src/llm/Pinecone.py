...
from pinecone import PineconeAsyncio as Pinecone, IndexModel
...
class PineConeConfig(BaseModel):
    ...
    default_dimension: int = Field(default=1024, ge=1)  # Ensuring it's positive
...
class PineconeOperations:
    ...
    __slots__ = (
        '_pc', '_spec', '_current_index_host', '_current_index','_current_index_name', 
        '_default_dense_model', '_default_sparse_model', '_default_dimension'
    )
    ...
    def __init__(self, config: PineConeConfig, **kwargs):
        ...
        if not self.__from_create:
            raise RuntimeError("Use PineconeOperations.create() to instantiate this class.")
    ...
    async def create(cls, config: Optional[PineConeConfig]=None, **kwargs):
        ...
        if not api_key:
            raise ValueError("PINECONE_API_KEY is required to initialize Pinecone.")
        
        cls.__from_create = True   
        instance = cls(config, **kwargs)
        instance._pc = Pinecone(api_key=api_key, pool_threads=config.max_pool_threads)
        return instance
    ...
    async def set_current_index(self, index_name: str, index_host:str=None) -> bool:
        """
        ...
        self._current_index = self._pc.IndexAsyncio(host=self._current_index_host)
        return True
    ...
    def make_id(self,metadata:Dict, chunk_num:int, batch_num:Optional[int]) -> str:
        basename= str(os.path.basename(metadata.get("file_path", ""))).replace(" ", "-").removesuffix(".pdf")
    ...
    #currently , we are sending 90 documents in one batch request for embedding.
    ...
    async def dense_query(
        ...
        if isinstance(query, str):
            dense_vector = self.get_dense_embeddings([query], input_type="query")[0].get("values")
            query = {
                "vector": dense_vector,
                "top_k": top_k,
                "include_metadata": include_metadata
    ...
    async def hybrid_query(
        ...
        if isinstance(query, str):
            ...
            normalized_dense_vector, normalized_sparse_vector = self.hybrid_score_norm(dense_vector, sparse_vector, alpha=0.5)
            query = {
            ...
            query["include_metadata"] = query.get("include_metadata", include_metadata)
        else:
    ...
    async def rerank(self, query: str, matches: List[Dict], model:str="bge-reranker-v2-m3", top_n: int = 3) -> List[Dict]:
        """
