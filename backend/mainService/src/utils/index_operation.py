...
def start():
    # Start the background flushing thread (as a daemon so it doesn't block program exit)
    flush_thread = threading.Thread(target=flush_index_dict_periodically, args=(180,), daemon=True)
    flush_thread.start()
...
if __name__ == "__main__":
    ...
    # For demonstration, keep the script running so the background thread works.
    while True:
        time.sleep(10)
